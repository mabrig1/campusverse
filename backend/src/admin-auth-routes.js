...
async function bootstrapAdmin() {
  ...
  if (existing) {
    return prisma.user.update({
      ...
      data: { role: "ADMIN", username: existing.username || username, emailVerified: true },
    });
  }
...
router.post("/api/admin/auth/forgot-password", async (req, res) => {
  // Always return the same response so the endpoint cannot reveal whether an admin account exists.
  ...
  try {
    ...
    const user = await prisma.user.findFirst({
      where: { role: "ADMIN", OR: [{ username: identifier }, { email: identifier }] },
    });
    ...
    await prisma.$executeRawUnsafe(
      `INSERT INTO admin_password_resets (id,user_id,token_hash,expires_at) VALUES ($1,$2,$3,$4)`,
      crypto.randomUUID(),
      user.id,
      tokenHash,
      new Date(Date.now() + RESET_MINUTES * 60 * 1000)
    );
...
router.post("/api/admin/auth/reset-password", async (req, res) => {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe(
      `SELECT * FROM admin_password_resets WHERE token_hash=$1 AND used_at IS NULL AND expires_at > CURRENT_TIMESTAMP LIMIT 1`,
      tokenHash
    );
    const reset = rows[0];
